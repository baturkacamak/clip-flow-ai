...
from src.config_manager import ConfigManager, TranscriptionConfig
...
class AudioTranscriber:
    def __init__(self, config_manager: ConfigManager):
        ...
        logger.info(
            f"Initializing Whisper Model: {self.cfg.model_size} on {self.device} ({self.compute_type})"
        )

        self.model = WhisperModel(
            self.cfg.model_size, device=self.device, compute_type=self.compute_type
        )
    ...
    def _get_compute_type(self) -> str:
        if self.cfg.compute_type != "auto":
            if self.device == "cpu" and self.cfg.compute_type == "float16":
                logger.warning(
                    "Float16 requested on CPU, falling back to int8 for compatibility."
                )
                return "int8"
        ...
        try:
            ...
            for seg in segments_generator:
                ...
                if seg.avg_logprob < -1.0:  # Threshold can be tuned
                    logger.debug(
                        f"Low confidence segment ({seg.avg_logprob:.2f}): {seg.text}"
                    )
            ...
            logger.success(
                f"Transcription complete in {processing_time:.2f}s. Saved to {cache_path}"
            )
            return result
