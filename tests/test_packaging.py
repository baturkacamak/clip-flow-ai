...
def sample_clip():
    return ViralClip(
        ...
        title="Test Clip", virality_score=90, 
        reasoning="Good", category="Tech"
    )

def test_metadata_generator_mock(mocker, mock_config_manager, sample_clip):
    mocker.patch("src.packaging.generator.instructor.from_openai")
...
def test_thumbnail_maker_logic(mocker, mock_config_manager, sample_clip):
        # Mock cv2 and PIL
        mock_cv2 = mocker.patch("src.packaging.thumbnail.cv2")
        # Mock Image
        mock_image = mocker.patch("src.packaging.thumbnail.Image")
        mock_pil_img = mocker.Mock()
        mock_pil_img.size = (1920, 1080)
        mock_image.fromarray.return_value = mock_pil_img
        
        mocker.patch("src.packaging.thumbnail.ImageDraw")
        mocker.patch("src.packaging.thumbnail.ImageFont")
