...
def mock_components(mocker):
    ...
    mocker.patch("python_core.pipeline.VideoDownloader")
    mocker.patch("python_core.pipeline.AudioTranscriber")
    mocker.patch("python_core.pipeline.ContentCurator")
    mocker.patch("python_core.pipeline.LibraryIndexer")
    mocker.patch("python_core.pipeline.VisualMatcher")
    mocker.patch("python_core.pipeline.SmartCropper")
    mocker.patch("python_core.pipeline.VideoCompositor")
    mocker.patch("python_core.pipeline.SubtitleOverlay")
    mocker.patch("python_core.pipeline.MetadataGenerator")
    mocker.patch("python_core.pipeline.ThumbnailMaker")
    mocker.patch("python_core.pipeline.YouTubeUploader")
    mocker.patch("python_core.pipeline.TikTokUploader")
...
def test_pipeline_run_flow(mocker, mock_config_manager, mock_components):
    ...
    mock_matcher = mocker.patch("python_core.pipeline.VisualMatcher").return_value
    mock_matcher.find_match.return_value = "b_roll.mp4"

    mock_cropper = mocker.patch("python_core.pipeline.SmartCropper").return_value
    ...
    # Mock Metadata
    mock_meta_gen = mocker.patch("python_core.pipeline.MetadataGenerator").return_value
    ...
    mock_yt_cls = mocker.patch("python_core.pipeline.YouTubeUploader")
