...
def sample_package():
    return VideoPackage(
        video_path="vid.mp4", thumbnail_path="thumb.jpg",
        title="Title", description="Desc", tags=["tag"], captions="Cap",
        platforms=["youtube", "tiktok"]
    )

def test_youtube_upload_flow(mocker, mock_config_manager, sample_package):
    # Mock googleapiclient
    mocker.patch("src.distribution.youtube.build")
    mocker.patch("src.distribution.youtube.InstalledAppFlow")
    mocker.patch("src.distribution.youtube.MediaFileUpload")
