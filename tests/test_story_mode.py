...
def mock_transcriber(mocker):
    mock = mocker.patch("python_core.modes.story_builder.AudioTranscriber")
...
def mock_matcher(mocker):
    mock = mocker.patch("python_core.modes.story_builder.VisualMatcher")
    instance = mock.return_value
...
def test_story_builder_no_audio(mock_config_manager, mocker):
    ...
    mocker.patch("python_core.modes.story_builder.AudioTranscriber")
    builder = StoryBuilder(mock_config_manager)
...
def test_pipeline_story_mode_execution(mocker, mock_config_manager, tmp_path):
    ...
    mock_builder_cls = mocker.patch("python_core.pipeline.StoryBuilder")
    mock_builder = mock_builder_cls.return_value
    mock_builder.build_plan.return_value = (MagicMock(), MagicMock())  # plan, transcript

    mock_compositor_cls = mocker.patch("python_core.pipeline.VideoCompositor")
    mock_overlay_cls = mocker.patch("python_core.pipeline.SubtitleOverlay")
